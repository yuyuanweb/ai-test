...
func main() {
	...
	api := r.Group("/api")
	{
		...
		conversation := api.Group("/conversation")
		{
			...
			conversation.POST("/battle/stream", middleware.AuthMiddleware(), middleware.AIStreamRateLimit(), conversationHandler.BattleStream)
			...
			conversation.GET("/battle/mapping", middleware.AuthMiddleware(), conversationHandler.GetBattleModelMapping)
